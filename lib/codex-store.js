...
function roleForEvent(event) {
  ...
  if (event.type === 'response_item') {
    ...
    if (payload.type === 'function_call_output') return 'tool-result';
...
export function getConversationItems(events) {
  const hasResponseMessages = events.some(event => {
    return event.type === 'response_item' && event.payload?.type === 'message';
  });
  ...
  for (let i = 0; i < events.length; i++) {
    ...
    // Some Codex sources may only write event_msg entries. If response_item
    // messages are present, event_msg user/agent messages are mirror events.
    if (!hasResponseMessages && event.type === 'event_msg' && (payload.type === 'user_message' || payload.type === 'agent_message')) {
      const text = messageText(payload);
      if (!text || isEnvironmentContext(text)) continue;
      items.push({
        id: `${i}-${event.timestamp || ''}`,
